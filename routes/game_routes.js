...
module.exports = function(_game) {
    ...
    var environment         = game.aux.trigger.target;
    ...
    var rExist = game.aux.rExist;
    var g      = game.aux.g;
    var un     = game.aux.un;

    var roomsExist  = game.aux.roomsExist;
    var isConnected = game.aux.isConnected;
    var loginR      = game.aux.loginR;
    var rooms       = game.aux.rooms;
    var rName       = game.aux.rName;
        ...
        game.io.on('connection', function(socket){
            socket.on('user_connected',function(m){
                ...
                if(m.player_number === '2'){
                    game.io.to(m.roomId).emit('activation'+m.roomId,{room:rooms});
                }
            });
            socket.on('moved'+req.params.gameId,function(m){
                game.io.to(req.params.gameId).emit('move'+req.params.gameId,m);
            });
            // FOR CHECKMATE
            socket.on('cm'+req.params.gameId,function(m){
                game.io.to(req.params.gameId).emit('checkMate'+req.params.gameId,m);
            });
            socket.on('disconnect',function(){
                    roo.find({},function(err,rous){
                        ...
                        game.io.to(room_id).emit('dcnt'+room_id,'dsº');
